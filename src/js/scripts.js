(function () {
  ...
  const
    mobileMenu = document.querySelector('.page-header__menu-box');

  function closeMenu() {
    mobileMenu.classList.remove('page-header__menu-box--active');
    showOverflowY()
  }

  function openMenu() {
    mobileMenu.classList.add('page-header__menu-box--active');
    hideOverflowY()
  }
  ...
  if (openMenuButton) {
    const
      toggleMenu = () => {
        if (mobileMenu.classList.contains('page-header__menu-box--active')) {
          closeMenu()
        } else {
          openMenu()
        }
